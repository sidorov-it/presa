...
> [!NOTE]
> По умолчанию приложение записывает файлы логов в каталог `../logs` относительно корня проекта. Это гарантирует, что логи сохраняются вне папки приложения и не удаляются при деплое новой версии. Путь можно изменить через переменную окружения `LOG_DIR`. Для относительных путей используется родительский каталог приложения.
