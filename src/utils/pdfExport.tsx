...
export const exportPresentationToPdf = async (
    ...
    // Set default options
    const { paperSize = 'a4', orientation = 'landscape', includeSlideNumbers = true, scale = 2 } = options;
    ...
    try {
        ...
        for (let i = 0; i < totalSlides; i++) {
            ...
            const slideElement = await renderSlideForExport(presentation.id, slide, container);
...
const renderSlideForExport = async (
    ...
    root.render(<SlideViewer slide={slide} presentationId={presentationId} />);

    await new Promise(resolve => setTimeout(resolve, 100)); // немного подождать, чтобы отрендерилось
